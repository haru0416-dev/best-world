...
- 電媒体 F0: Filament は reflectance \(r\)（既定 0.5）から

\[
F_{0,\mathrm{diel}} = 0.16\, r^2
\]

\(r=0.5\) で \(F_0=0.04\)。これは IOR 1.5 の空気–電媒体界面に対応します。
...
Filament は \(F_{90}=\mathrm{saturate}(\langle F_0,\,(50\cdot 0.33,50\cdot 0.33,50\cdot 0.33)\rangle)\) を電媒体ローブに使います。輝度近似で、典型的な \(F_0=0.04\) では \(F_{90}=1\) になります。\(F_0\) が極端に暗い（スペキュラオクルージョンを F0 に焼いた）面では grazing も落ちます。金属ローブは OpenPBR の F82-tint で、その中の Schlick は \(F_{90}=1\) です。
...
`specular_color = 1` なら補正項は 0 で Schlick に戻ります。電媒体ローブには掛けず、金属側だけ使います。
...
炉試験で金属・電媒体ともエネルギーが保たれます。F82 は直接光の金属ローブだけに使い、IBL の DFG は LUT を持たないため Schlick のままです。
...
| 電媒体 \(F_{90}\) | スカラー `dot(f0, 16.5)` | Filament: `dot(f0, vec3(50·0.33))` |
